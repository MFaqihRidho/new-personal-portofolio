<template>
  <div class="grid grid-cols-6 gap-x-28 gap-y-16">
    <img src="https://img.icons8.com/color/144/000000/html-5--v1.png" />
    <img src="https://img.icons8.com/color/144/000000/css3.png" />
    <img src="https://img.icons8.com/color/144/000000/javascript--v1.png" />
    <img src="https://img.icons8.com/color/144/000000/tailwindcss.png" />
    <img src="https://img.icons8.com/color/144/000000/bootstrap.png" />
    <img src="https://img.icons8.com/color/144/000000/material-ui.png" />
    <img src="https://img.icons8.com/color/144/000000/react-native.png" />
    <img src="https://img.icons8.com/color/144/000000/vue-js.png" />
    <img src="https://img.icons8.com/color/144/000000/nextjs.png" />
    <img src="https://img.icons8.com/color/144/000000/redux.png" />
    <img src="https://img.icons8.com/color/144/000000/nodejs.png" />
    <img src="https://img.icons8.com/color/144/000000/mongodb.png" />
    <img src="https://img.icons8.com/color/144/000000/git.png" />
    <img src="https://img.icons8.com/color/144/000000/github--v3.png" />
    <img
      src="https://img.icons8.com/color/144/000000/visual-studio-code-2019.png"
    />
  </div>
</template>